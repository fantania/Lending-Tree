...
fa-angle-down"></i></a>
	...
	<div class="breadcumb-area flex-style  black-opacity">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<h2>Assigned  Loan Request List</h2>
	...
	<div class="shop-area">
		...
		<div class="container">
		<h2 style="color: #ff7733;">Assigned  Loan Request List</h2>
			<div class="row">
			<div class="shop-wrap">
		<table class="table table-striped">
							...
							<tbody>
								<c:forEach items="${loanList}" var="loan">
									<tr>
										<td>${loan.empIdLoan}</td>
										<td>${loan.customerIdentifier}</td>
										<td>${loan.loanId}</td>
										<td>${loan.loanType}</td>
										<td>${loan.bankName}</td>
										<td>${loan.documentsList}</td>
										<td>${loan.customerAddress}</td>
										<td>${loan.status}</td>
										<td>${loan.loanDate}</td>
										<td><spring:url value="/upLoadDocs/${loan.empIdLoan}/${loan.loanId}"
												var="assignURL" />
												
											<div id="assignBtn"style=" background-color: #ff7733;  border-radius: 5px; width: 100px; height: 50px; text-align:center; color: #FFF"><a href="${assignURL}" role="button">UpLoad Documents</a></div>
										</td>
									</tr>
								</c:forEach>
								<tr><td><div><spring:url value="/login"
                                                         								var="logout" /><a class="btn btn-danger" href="${logout}"
                                                         							role="button">Log Out</a></div></td></tr>
							</tbody>
						</table>
	</div>
